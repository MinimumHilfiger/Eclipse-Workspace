...
public class Steuerung {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(
				"Steuerung : Norden = n , Osten = o , Süden = s , Westen = w , zum Beenden des spiels = beenden ");
		System.out.println("Gewinner des Spiels ist der, der zuerst mindestens 65 Punkte hat! ");

		Spieler weiß = new Spieler("W", true); // 2 Objekte der Spieler-Klasse werden erstellt
		...
		Oberflaeche.initialisiereSpieler(weiß, schwarz);

		while (weiterSpielen == true) { // Schleife für die wiederholende Ausführung der Bewegungsmethode der Spieler
			if (weiß.istDran) { // läuft solange bis jemand gewonnen hat oder das programm beendet wurde
				zugZahl++;
				Oberflaeche.bewegungsVerarbeitung(weiß, schwarz);
			} else {
				zugZahl++;
				Oberflaeche.bewegungsVerarbeitung(schwarz, weiß);
			}
		}
	}

	public static void punkteRechner(Spieler spieler, Feld feld) { // Methode für die Berechnung der Punktzahl
		...
		if (spieler.erstesMal) { // Wenn das erste Mal auf ein Feld gesprungen wird ist die Berechnung der
			...
			// gesprungen wird
			punktZahl = punktZahl.add(feldBruch); // Beide brüche werden addiert
			punktZahl.kuerzen(); // Brüche werden gekürtzt falls möglich
			...
			punkteWiedergabe(punktZahl, spieler); // Methode zur punktWiedergabe wird ausgerufen
	...
	public static void punkteWiedergabe(FractionMax pZ, Spieler spieler) { // Methode welche die Punkte der Spieler
																			// ausgibt und überprüft ob jemand gewonnen
																			// hat

		if (pZ.getZaehler().compareTo(pZ.getNenner()) == 1) { // Es wird unterschieden ob der Zähler der Punktzahl
																// größer ist als der Nenner
